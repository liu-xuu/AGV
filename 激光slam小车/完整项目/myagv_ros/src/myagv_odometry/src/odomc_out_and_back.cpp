...
int main(int argc, char** argv)
{
  ...
  double angular_tolerance = 0; //½Ç¶È×ª»»³É»¡¶È:deg*PI/180
  ...
  for(int i = 0;i < 2;i++)
  {
    ...
    while( (fabs(turn_angle + angular_tolerance) < M_PI) && (ros::ok()) )
    {
        ...
        listener.lookupTransform(odom_frame, base_frame, ros::Time(0), transform);
        //C++: abs()ÇóµÃÊÇÕýÊýµÄ¾ø¶ÔÖµ,fabs()ÇóµÃÊÇ¸¡µãÊýµÄ¾ø¶ÔÖµ;python:abs(x),²ÎÊý¿ÉÒÔÊÇ£º¸ºÊý¡¢ÕýÊý¡¢¸¡µãÊý»òÕß³¤ÕûÐÎ
