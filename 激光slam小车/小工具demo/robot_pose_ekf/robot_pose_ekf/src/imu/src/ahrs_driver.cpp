...
ahrsBringup::ahrsBringup() :frist_sn_(false), serial_timeout_(20)
{
  ...
  pravite_nh.param("imu_topic", imu_topic_, std::string("/imu_data"));
  pravite_nh.param("imu_frame", imu_frame_id_, std::string("imu_link")); 
  ...
  //发送端口是否打开情况
  driver_Publisher = nh_.advertise<std_msgs::String>("/serial_driver", 10);
  //setp up serial
  try
  {
  ...
  catch (serial::IOException &e)
  {
    ROS_ERROR_STREAM("Unable to open port ");
     //1.imu 串口
        ...
        msgdata.data = "Unable to open port  imu/ahrs_driver.cpp imu串口未能打开请检查！";
        driver_Publisher.publish(msgdata);
    exit(0);
  }
  if (serial_.isOpen())
  {
   ...
   //1.imu 串口
        std_msgs::String msgdata;
        msgdata.data = "Serial Port initialize imu/ahrs_driver.cpp imu串口打开成功！";
        driver_Publisher.publish(msgdata);
  }
  else
  {
    ROS_ERROR_STREAM("Unable to initial Serial port ");
      //1.imu 串口
        std_msgs::String msgdata;
        msgdata.data = "Unable to initial Serial port imu/ahrs_driver.cpp imu串口未能打开请检查！";
        driver_Publisher.publish(msgdata);
    exit(0);
  }
  processLoop();
  ...
  while (ros::ok())
  {
    ...
    if (head_type[0] == TYPE_AHRS)
    {
      ...
      else if (device_type_ == 1)    //imu单品ROS标准下的坐标变换
      {
        ...
        imu_data.angular_velocity.y = -ahrs_frame_.frame.data.data_pack.PitchSpeed;
          // imu_data.angular_velocity.z = -ahrs_frame_.frame.data.data_pack.HeadingSpeed;
        imu_data.angular_velocity.z = ahrs_frame_.frame.data.data_pack.HeadingSpeed;//sukai
        imu_data.linear_acceleration.x = -imu_frame_.frame.data.data_pack.accelerometer_x;
        imu_data.linear_acceleration.y = imu_frame_.frame.data.data_pack.accelerometer_y;
        imu_data.linear_acceleration.z = imu_frame_.frame.data.data_pack.accelerometer_z;
      }
